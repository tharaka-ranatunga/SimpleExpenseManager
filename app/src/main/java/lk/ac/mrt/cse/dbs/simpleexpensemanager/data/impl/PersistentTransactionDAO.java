...
public class PersistentTransactionDAO implements TransactionDAO {
    ...
    public List<Transaction> getAllTransactionLogs() {
        ...
        if(result.moveToFirst()) {
            do{
                Transaction t = new Transaction(new Date(result.getLong(result.getColumnIndex("Log_date"))), result.getString(result.getColumnIndex("Account_no")), (result.getInt(result.getColumnIndex("Type")) == 0) ? ExpenseType.EXPENSE : ExpenseType.INCOME, result.getDouble(result.getColumnIndex("Amt")));
                transactions.add(t);
